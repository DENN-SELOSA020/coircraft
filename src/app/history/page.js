...
export default function HistoryPage() {
  const { user, markReceived } = useApp();
                  ...
                  <div key={tx.id} style={{ background: "#fff", borderRadius: 20, overflow: "hidden", boxShadow: "0 4px 20px rgba(14,32,17,0.06)" }}>
                    ...
                    <div style={{ padding: "20px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
                      <div>
                        ...
                        <div style={{ fontSize: 12, color: "#aaa" }}>{new Date(tx.created_at).toLocaleString()}</div>
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                        <div style={{ fontSize: 12, color: "#888" }}>{tx.payment_method} · {tx.delivery}</div>
                        <div style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 18, color: "#0E2011" }}>₱{Number(tx.total).toLocaleString()}</div>
