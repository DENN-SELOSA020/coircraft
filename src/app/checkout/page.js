...
export default function CheckoutPage() {
  ...
  const shipping = useMemo(() => (delivery === "Pickup" ? 0 : 120), [delivery]);
  const total = useMemo(() => subtotal + shipping, [subtotal, shipping]);

  const selectedPayment = useMemo(() => paymentMethods.find((p) => p.id === payment), [payment]);
  ...
  const placeOrder = async () => {
    ...
    // Ensure transaction is attached to the logged-in user
    const currentUserEmail = user.email;
    if (currentUserEmail !== user.email) {
      alert("Account mismatch detected. Please log in with your account.");
      return;
    }
  ...
  if (done) return (
    <div style={{ minHeight: "100vh", background: "#FFFEF5", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-body)" }}>
      <div style={{ textAlign: "center" }} className="animate-bounceIn">
        ...
        <h2 style={{ fontFamily: "var(--font-display)", fontSize: 28, fontWeight: 800, color: "#000000", margin: "0 0 8px" }}>Order Placed!</h2>
    ...
    <>
      ...
      <main style={{ background: "#FFFEF5", minHeight: "100vh", fontFamily: "var(--font-body)", padding: "40px 24px 80px" }}>
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          <h1 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(24px,4vw,36px)", fontWeight: 800, color: "#000000", margin: "0 0 28px" }}>Checkout</h1>
              ...
              {step === 3 && (
                <div style={{ background: "#fff", borderRadius: 22, padding: "28px", boxShadow: "0 4px 20px rgba(14,32,17,0.06)" }} className="animate-fadeSlideUp">
                  ...
                  <div style={{ display: "flex", gap: 10 }}>
                    ...
                    <button className="tk-btn-cta" style={{ flex: 2, textAlign: "center" }} onClick={placeOrder} disabled={loading}>
