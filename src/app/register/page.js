  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const apiErr = data.error || "Server registration failed";
        setError(apiErr + ". Falling back to local registration...");
        const fallback = fallbackLocalRegister();
        if (!fallback) return;
      } else {
        setSuccess(true);
